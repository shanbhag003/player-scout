/* Cricket. A separate page from football on purpose: the two sports share a
   shell, a stylesheet and a switcher, but not a code path. Football's app.js is
   sixty-eight kilobytes of working, validated code and there is no reason for a
   new sport to reach into it.

   Data is fetched rather than inlined, and the three files are the same ones the
   pipeline writes. */
(async function(){
...
let D;
try {
  const [index, detail, meta] = await Promise.all(
    ["index","detail","meta"].map(n =>
      fetch(`${BASE}/${n}.json`).then(r => {
        if (!r.ok) throw new Error(`${n}.json ${r.status}`);
        return r.json();
      })));
  // Reshape into the compact form the view works with, so the rest of this file
  // does not care how the pipeline happens to lay its json out.
  D = {
    players: index.map(e => {
...
function select(pid){
  ...
  $("sugg").hidden=true; $("q").value=e[0].f||e[0].n; draw();
}
...
function paneProfile(){
  ...
  return `<div class="card"><h2>${p.f||p.n}</h2>
...
function paneSimilar(){
  ...
  return head+list.map(([s,c])=>`<div class="row">
    ...
    <div><div class="who2">${c.f||c.n}</div><div class="why">${why(p,c)}</div></div>
...
function paneCompare(){
  ...
  return `<div class="chips">${set.map(x=>`<span class="chip">${x.f||x.n}
